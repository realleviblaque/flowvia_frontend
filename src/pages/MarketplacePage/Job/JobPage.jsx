...
export function JobPage({all, handleDialogOpen, dialog, handleDialogClose, plusDialog, hadnlePlusDialogOpen, hadnlePlusDialogClose}) {
  ...
  useEffect(() => {
    const checkSearch = () => {
      if (search) {
        const filtered = job.filter(p => p.clinetUsername.toLowerCase().includes(search.toLowerCase()) || p.jobTitle.toLowerCase().includes(search.toLowerCase()) || p.bio.toLowerCase().includes(search.toLowerCase()) || p.skill.toLowerCase().includes(search.toLowerCase()))
        setJobs(filtered)
      } else {
        setJobs(job)
      }
